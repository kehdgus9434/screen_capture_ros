...
class DirectCapturePublisher(Node):
    def __init__(self):
        super().__init__('direct_capture_publisher')
        
        # 파라미터 선언
        self.declare_parameter('window_names', ['Camera', 'Map'])  # 캡처할 창 이름 목록
        ...
        for window_name in self.window_names:
            self.window_info[window_name] = {
                ...
                'temp_file': os.path.join(self.temp_dir, f"capture_{self.get_safe_name(window_name)}.png"),
        ...
        self.get_logger().info(
            f'DirectCapturePublisher 초기화 완료: '
    ...
    def capture_and_publish_thread(self, window_name):
        ...
        while rclpy.ok() and self.running:
            ...
            try:
                # 창 캡처
                with lock:
                    success = self.capture_screenshot(info['window_id'], info['temp_file'])
                ...
                # 이미지 로드
                image = cv2.imread(info['temp_file'])
                if image is None:
                    self.get_logger().warn(f"이미지 로드 실패: {info['temp_file']}")
                    time.sleep(wait_time)
                    continue
    ...
    def capture_screenshot(self, window_id, output_file):
        """간단하고 안정적인 스크린샷 캡처"""
        try:
            # "import" 명령으로 스크린샷 캡처 (ImageMagick)
            result = subprocess.run(
                ['import', '-silent', '-window', window_id, output_file], 
            ...
            if result.returncode != 0:
                ...
                window_geom = subprocess.check_output(
                    ['xwininfo', '-id', window_id], text=True
                )
                
                # 창 위치와 크기 파싱
                x = y = width = height = None
                for line in window_geom.splitlines():
                ...
                if x is not None and y is not None and width is not None and height is not None:
                    # scrot로 스크린샷 캡처
                    subprocess.run(
                        ['scrot', '-a', f'{x},{y},{width},{height}', output_file],
                        check=True
                    )
                else:
                    return False
            ...
            self.get_logger().error(f"스크린샷 캡처 오류: {str(e)}")
            return False
