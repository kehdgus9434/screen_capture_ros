...
class PipeScreenCapturePublisher(Node):
    def __init__(self):
        super().__init__('pipe_screen_capture_publisher')
        ...
        self.get_logger().info(
            f'PipeScreenCapturePublisher 초기화 완료: '
    ...
    def capture_and_publish_thread(self, window_name):
        ...
        while rclpy.ok() and self.running:
            ...
            try:
                ...
                with lock:
                    # 이미지 캡처 시도
                    image = self.capture_through_pipe(info['window_id'])
    ...
    def capture_through_pipe(self, window_id):
        """파이프를 통해 디스크 I/O 없이 메모리에서 직접 창 캡처"""
        try:
            # 방식 1: import에서 직접 PNG 출력을 파이프로 수신
        ...
        try:
            # 방식 2: 파이프 연결로 디스크 I/O 없이 이미지 전달
            ...
            # 첫 번째 프로세스의 출력 파이프 닫기 (파이프라인 동작을 위해)
            xwd_process.stdout.close()
            
            # 시간 초과 설정 (0.5초)
            try:
                stdout, _ = convert_process.communicate(timeout=0.5)
            except subprocess.TimeoutExpired:
            ...
            # 결과 확인
            if convert_process.returncode == 0 and len(stdout) > 0:
                # 이미지 데이터를 메모리에서 바로 디코딩
                nparr = np.frombuffer(stdout, np.uint8)
                image = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
                return image
        ...
        try:
            # 방식 3: 메모리에서 직접 처리하는 PIL 사용 (파이프로 캡처)
            xwd_process = subprocess.Popen(
                ['xwd', '-silent', '-id', window_id],
                stdout=subprocess.PIPE,
                stderr=subprocess.DEVNULL
            ...
            # 시간 초과 설정 (0.5초)
            try:
                stdout, _ = xwd_process.communicate(timeout=0.5)
            except subprocess.TimeoutExpired:
                xwd_process.kill()
                return None
            
            if xwd_process.returncode == 0 and len(stdout) > 0:
                # XWD 데이터를 메모리 버퍼에 저장
                buffer = io.BytesIO(stdout)
                
                # XWD 데이터를 PIL 이미지로 변환 (디스크 접근 없음)
                try:
                    from PIL import Image as PILImage
        ...
        # 모든 방식이 실패한 경우 마지막 수단: scrot으로 화면 영역 캡처
        try:
            # 윈도우 위치와 크기 가져오기
            ...
            if x is not None and y is not None and width is not None and height is not None:
                # scrot으로 영역 캡처하여 파이프로 출력
                scrot_process = subprocess.Popen(
                    ['scrot', '-a', f'{x},{y},{width},{height}', '-', '-z'],
                    stdout=subprocess.PIPE, 
                    stderr=subprocess.DEVNULL
                )
                ...
                # 시간 초과 설정 (0.5초)
                try:
                    stdout, _ = scrot_process.communicate(timeout=0.5)
                except subprocess.TimeoutExpired:
                    scrot_process.kill()
                    return None
                
                if scrot_process.returncode == 0 and len(stdout) > 0:
                    # 이미지 데이터를 메모리에서 바로 디코딩
                    nparr = np.frombuffer(stdout, np.uint8)
                    image = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
                    return image
...
def main(args=None):
    ...
    try:
        # 노드 생성
        node = PipeScreenCapturePublisher()
